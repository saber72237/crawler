...
<div class="navbar navbar-inverse navbar-fixed-top">
    <div class="navbar-inner">
        <div class="container">
            ...
            <a class="brand" href="index.php" style="padding: 8px 20px 0;cursor: pointer;" title="刷新并回到首页">数据中心</a>
    ...
    <div class="search"  style="text-align:right;">
        ...
        <div class = "form"; style="display:block;">
            <table class='table table-striped table-bordered table-hover'>
                ...
                <tr>
                    <th style="text-align: center">灯号</th>
                    <th style="text-align: center">亮度</th>
                    <th style="text-align: center">车流量</th>
                    <th style="text-align: center">温度</th>
                    <th style="text-align: center">污染指数</th>
                    <th style="text-align: center">上传日期</th>
                </tr>
                 <?php
                  
                    foreach($array as $key=>$values){
                        $urlName = urlencode($values->name);
                        echo "<tr>";
                        echo "<td style=\"text-align: center\">{$values->ID}</td>";
                        echo "<td style=\"text-align: center\">{$values->name}</td>";
                        echo "<td style=\"text-align: center\">{$values->class}</td>";
                        echo "<td style=\"text-align: center\">{$values->grade}</td>";
                        echo "<td style=\"text-align: center\">{$values->size}</td>";
                        echo "<td style=\"text-align: center\">{$values->date}</td>";
                        echo "</tr>";
                        ;
                    }
                    ?>
